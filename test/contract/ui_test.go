...
func TestUI_LoginRejectsUnsupportedMethod(t *testing.T) {
	srv := newUITestServer(t)
...
func TestUI_Logout(t *testing.T) {
	...
	dashboardReq := httptest.NewRequest(http.MethodGet, "/ui/", nil)
	dashboardReq.AddCookie(cookie)
	dashboardW := httptest.NewRecorder()
	srv.ServeHTTP(dashboardW, dashboardReq)
	require.Equal(t, http.StatusOK, dashboardW.Code)
	buttonType, found := logoutButtonType(dashboardW.Body.String())
	require.True(t, found, "logout form button should be rendered")
	assert.Equal(t, "submit", buttonType)
	...
	require.True(t, cleared, "logout must return an expired session cookie")

	replayReq := httptest.NewRequest(http.MethodGet, "/ui/", nil)
	replayReq.AddCookie(cookie)
	replayW := httptest.NewRecorder()
	srv.ServeHTTP(replayW, replayReq)
	assert.Equal(t, http.StatusSeeOther, replayW.Code)
	assert.Equal(t, "/ui/login", replayW.Header().Get("Location"))
}

func logoutButtonType(document string) (string, bool) {
	root, err := html.Parse(strings.NewReader(document))
	if err != nil {
		return "", false
	}

	var findButton func(*html.Node, bool) (string, bool)
	findButton = func(node *html.Node, inLogoutForm bool) (string, bool) {
		if node.Type == html.ElementNode && node.Data == "form" {
			inLogoutForm = false
			for _, attr := range node.Attr {
				if attr.Key == "action" && attr.Val == "/ui/logout" {
					inLogoutForm = true
					break
				}
			}
		}
		if inLogoutForm && node.Type == html.ElementNode && node.Data == "button" {
			for _, attr := range node.Attr {
				if attr.Key == "type" {
		...
		for child := node.FirstChild; child != nil; child = child.NextSibling {
			if buttonType, ok := findButton(child, inLogoutForm); ok {
