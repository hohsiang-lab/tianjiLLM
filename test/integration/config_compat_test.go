...
func TestConfig_FullPythonCompatibility(t *testing.T) {
	...
	assert.Equal(t, []any{"langfuse"}, cfg.TianjiSettings.Overflow["success_callback"])
	assert.Equal(t, []any{"slack"}, cfg.TianjiSettings.Overflow["failure_callback"])
	...
	assert.Equal(t, true, cfg.Guardrails[0].Overflow["default_on"])
}
