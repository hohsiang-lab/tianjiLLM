...
// TestNativeProxyStreamingSpendLog_GzipResponse is a regression test for HO-60.
//
// The real root cause: Anthropic returns gzip-compressed SSE when the client
// sends Accept-Encoding: gzip. Go's ReverseProxy passes through compressed
// bytes without decoding, so sseSpendReader's buffer contains gzip data and
// parseSSEUsage can't find "data: " prefixed lines → tokens=0.
func TestNativeProxyStreamingSpendLog_GzipResponse(t *testing.T) {
	// 1. Fake upstream that returns GZIP-compressed SSE
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	// 3. Real TCP server with chi middleware
	r := chi.NewRouter()
	r.Use(chiMiddleware.Recoverer)
	...
	// 4. Request with Accept-Encoding: gzip (like real browsers/clients)
	payload := map[string]any{
		"model":      "claude-opus-4-6",
		"max_tokens": 10,
		"stream":     true,
	...
	// Use raw transport to prevent auto-decompression by http.Client
	...
	// 6. Verify — without gzip decompression in Close(), tokens would be 0
	t.Logf("LogData: model=%s prompt=%d completion=%d",
		gotLogData.Model, gotLogData.PromptTokens, gotLogData.CompletionTokens)
