...
func TestStandardOpenAIRoutesRequireAuthentication(t *testing.T) {
	srv := proxy.NewServer(proxy.ServerConfig{
	...
	tests := []struct {
		...
		{http.MethodPost, "/v1/chat/completions", `{"model":"gpt-4o","messages":[]}`},
		{http.MethodPost, "/v1/embeddings", `{"model":"gpt-4o","input":"hi"}`},
		{http.MethodPost, "/v1/responses", `{"model":"gpt-4o","input":"hi"}`},
	}
