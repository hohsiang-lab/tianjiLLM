...
const usageNavigationTimeout = 60000.0

func (f *Fixture) NavigateToUsage() {
	f.T.Helper()
	_, err := f.Page.Goto(testServer.URL+"/ui/usage", playwright.PageGotoOptions{
		Timeout: playwright.Float(usageNavigationTimeout),
	})
	require.NoError(f.T, err)
	require.NoError(f.T, f.Page.WaitForLoadState())
	require.NoError(f.T, f.Page.Locator("#usage-content").WaitFor(playwright.LocatorWaitForOptions{
...
func TestUsage_DateRange_PresetChange(t *testing.T) {
	...
	f.SeedSpendLog(SeedSpendLogOpts{RequestID: "req-usage-old", Model: "openai/gpt-4o", Spend: 10.0, Tokens: 1000, StartAge: 15 * 24 * time.Hour})
	// Seed a log 2 days ago (inside 7d window)
	f.SeedSpendLog(SeedSpendLogOpts{RequestID: "req-usage-recent", Model: "openai/gpt-4o", Spend: 5.0, Tokens: 500, StartAge: 2 * 24 * time.Hour})
	f.NavigateToUsage()
