...
func TestKeyEdit_CancelReturnsToView(t *testing.T) {
	...
	// Click Cancel — view mode renders the alias in a settingsRow,
	// so wait for it to appear as proof that the HTMX swap completed.
	f.ClickButtonIn("#settings-content", "Cancel")
	f.WaitForTextIn("#settings-content", "edit-cancel")
