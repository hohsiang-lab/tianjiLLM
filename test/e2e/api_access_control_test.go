...
// Model metadata compatibility: legacy model access_control is ignored.

func TestAPI_ModelNew_IgnoresLegacyAccessControl(t *testing.T) {
	setup(t)
	...
	// model_info remains available, but the removed access_control field is omitted.
	...
	assert.Equal(t, "chat", info["mode"])
	assert.NotContains(t, info, "access_control")
}
...
func TestAPI_ModelUpdate_DropsLegacyAccessControl(t *testing.T) {
	setup(t)
