...
func TestHO81_TopModels_TabSwitch_PreservesData(t *testing.T) {
	...
	// Verify topModelsData is present on initial load.
	// Use state=attached because <script> tags are not visible.
	require.NoError(t, f.Page.Locator("#topModelsData").WaitFor(playwright.LocatorWaitForOptions{
		State:   playwright.WaitForSelectorStateAttached,
		Timeout: playwright.Float(defaultWaitTimeout),
	}), "topModelsData must be present on initial Cost tab load")

	// Switch away to Model Activity tab.
	require.NoError(t, f.Page.Locator(`button[data-tab="model-activity"]`).Click())
	f.WaitStable()

	// Switch back to Cost tab.
	require.NoError(t, f.Page.Locator(`button[data-tab="cost"]`).Click())

	// Wait for the Cost tab content to fully render (HTMX swap).
	require.NoError(t, f.Page.Locator("#topModelsData").WaitFor(playwright.LocatorWaitForOptions{
		State:   playwright.WaitForSelectorStateAttached,
		Timeout: playwright.Float(defaultWaitTimeout),
