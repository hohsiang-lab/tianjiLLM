...
func TestKeyCreate_FullLifecycle(t *testing.T) {
	...
	// 5. Raw key should be in new format (sk-ant-oat01-tianji-...)
	rawKey, err := f.Page.Locator(".select-all").TextContent()
	require.NoError(t, err)
	assert.True(t, strings.HasPrefix(rawKey, "sk-ant-oat01-tianji-"), "raw key should start with sk-ant-oat01-tianji-, got: %s", rawKey)
	assert.GreaterOrEqual(t, len(rawKey), 80, "raw key length should be >= 80, got %d", len(rawKey))
