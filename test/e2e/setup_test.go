...
const masterKey = "[REDACTED]"

type e2eSharedMemoryCache struct{ *cache.MemoryCache }

func (e2eSharedMemoryCache) SharedCoordinationAvailable() bool { return true }
...
func TestMain(m *testing.M) {
	...
	sharedCache := e2eSharedMemoryCache{MemoryCache: cache.NewMemoryCache()}
	uiHandler = &ui.UIHandler{
		DB:             testDB,
		Pool:           testPool,
		Config:         cfg,
	...
	proxyHandler = &handler.Handlers{
		Config: cfg,
		DB:     testDB,
		Cache:  uiHandler.Cache,
	}
	uiHandler.RefreshRuntimeModels = proxyHandler.RefreshRuntimeModels
