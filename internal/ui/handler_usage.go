...
func activeTab(r *http.Request) string {
	...
	switch tab {
	case "cost", "model-activity", "key-activity", "endpoint-activity", "claude-code", "codex":
		return tab
	default:
		return "cost"
	}
...
func (h *UIHandler) handleUsage(w http.ResponseWriter, r *http.Request) {
	...
	case "claude-code":
		tabContent = h.loadClaudeCodeData(r, h.dbReader())
	case "codex":
		tabContent = h.loadCodexUsageData(r)
	default:
