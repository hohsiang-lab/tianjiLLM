...
func (h *UIHandler) handleKeyDelete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.DB.DeleteVerificationToken(r.Context(), token); err != nil {
		data := h.loadKeysPageData(r)
		render(r.Context(), w, pages.KeysTableWithToast(data, "Failed to delete key: "+err.Error(), toast.VariantError))
		return
	}
