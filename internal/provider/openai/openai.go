...
func (p *Provider) UsesDefaultBaseURL() bool {
	return strings.TrimRight(strings.TrimSpace(p.baseURL), "/") == defaultBaseURL
}
...
func (p *Provider) GetSupportedParams() []string {
	if !p.UsesDefaultBaseURL() {
		return CompatibleSupportedParams()
	}
...
func transformRequestBody(req *model.ChatCompletionRequest) map[string]any {
	...
	// Merge provider-specific extra params for pass-through.
	// Skip keys already set to prevent ExtraParams from overriding known fields
	// such as model, messages, etc.
...
func parseErrorResponse(resp *http.Response) error {
	...
	if readErr != nil {
		return &model.TianjiError{StatusCode: resp.StatusCode, Message: fmt.Sprintf("upstream error (status %d): failed to read response body: %v", resp.StatusCode, readErr)}
	}
