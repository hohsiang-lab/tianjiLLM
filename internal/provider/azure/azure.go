...
func (p *Provider) GetSupportedParams() []string {
	if p.SupportsStandardCapabilities(nil) {
		return openai.SupportedParams
	}
	return openai.CompatibleSupportedParams()
}

func (p *Provider) SupportsStandardCapabilities(apiVersionOverride *string) bool {
	apiVersion := p.apiVersion
	if apiVersionOverride != nil {
		apiVersion = *apiVersionOverride
	}
	return strings.TrimSpace(apiVersion) == defaultAPIVersion
}
