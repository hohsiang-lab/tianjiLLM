...
func (h *Handlers) KeyRegenerate(w http.ResponseWriter, r *http.Request) {
	...
	oldHash := auth.HashKey(req.Key)
	newRaw := auth.GenerateVirtualKey()
	newHash := auth.HashKey(newRaw)
...
func (h *Handlers) KeyHealthCheck(w http.ResponseWriter, r *http.Request) {
	...
	token, err := h.DB.GetVerificationToken(r.Context(), auth.HashKey(key))
	if err != nil {
