...
func TestProxyHandlersNoConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{"Completion_NoModel", "POST", `{"model":"openaicompat/gpt-4o","prompt":"hi"}`, h.Completion},
		{"Embedding_NoModel", "POST", `{"model":"openaicompat/gpt-4o","input":"hello"}`, h.Embedding},
		{"ImageGeneration_NoModel", "POST", `{"model":"dall-e-3","prompt":"cat"}`, h.ImageGeneration},
		{"AudioSpeech_NoModel", "POST", `{"model":"openaicompat/tts-1","input":"hi","voice":"alloy"}`, h.AudioSpeech},
