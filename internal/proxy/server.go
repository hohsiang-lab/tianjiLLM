...
type Server struct {
	...
	budgetMW       func(http.Handler) http.Handler
...
// UIAuthRouter optionally registers social authentication routes outside /ui.
type UIAuthRouter interface {
...
func (s *Server) setupRoutes() {
	...
	// OpenAI OAuth callback (no API auth; state-authenticated)
	r.Route("/oauth/openai", func(r chi.Router) {
		r.Get("/callback", s.Handlers.OpenAIOAuthCallback)
	})

	// Claude Code CLI telemetry stub (no auth — telemetry should not require a key)
	r.Post("/api/event_logging/batch", s.Handlers.AnthropicEventLoggingBatch)
	...
	registerLLMRoutes := func(r chi.Router) {
		...
		r.Get("/responses", s.Handlers.CreateResponse)
		r.Post("/responses", s.Handlers.CreateResponse)
		r.Post("/responses/compact", s.Handlers.CompactResponse)
		r.Get("/models", s.Handlers.ListModels)
		r.Get("/models/{model}", s.Handlers.RetrieveModel)
	...
	r.Route("/credentials", func(r chi.Router) {
		...
		r.Post("/openai-subscription/{credential_id}/test", s.Handlers.OpenAISubscriptionCredentialTest)
		r.Post("/openai-subscription/{credential_id}/refresh", s.Handlers.OpenAISubscriptionCredentialRefresh)
		r.Post("/openai-subscription/{credential_id}/disable", s.Handlers.OpenAISubscriptionCredentialDisable)
		r.Post("/openai-subscription/{credential_id}/enable", s.Handlers.OpenAISubscriptionCredentialEnable)
	})
	...
	// Gemini native format (full middleware chain — same protection as /v1)
	...
	// Anthropic batches pass-through (full middleware chain — same protection as /v1)
	...
	if s.UIHandler != nil {
		...
		if authRouter, ok := s.UIHandler.(UIAuthRouter); ok {
