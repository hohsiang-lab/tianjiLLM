...
func TestCheckAndAlertOAuth_StatusRateLimited_TriggersAlert(t *testing.T) {
	...
	msgs := getMsgs()
	require.Len(t, msgs, 1, "alert should fire when UnifiedStatus == rejected")
	assert.Contains(t, msgs[0], "rejected", "alert should mention rejected status")
}
...
// HO-490: rejected status — exactly 1 alert (early return, no utilization alerts)
func TestCheckAndAlertOAuth_StatusRejected_TriggersAlert(t *testing.T) {
	srv, getMsgs := mockDiscordServerForOAuth(t, 200)
	defer srv.Close()
	...
	state := AnthropicOAuthRateLimitState{
		TokenKey:             "rejected-token-001",
		UnifiedStatus:        "rejected",
		Unified5hUtilization: 1.0,
		Unified7dUtilization: 1.0,
	}
	...
	msgs := getMsgs()
	require.Len(t, msgs, 1, "exactly 1 alert should fire for rejected status (early return)")
	assert.Contains(t, msgs[0], "rejected", "alert must mention rejected")
}

// HO-490: allowed_warning status — non-blocking alert (continues to util checks)
func TestCheckAndAlertOAuth_AllowedWarning_TriggersAlert(t *testing.T) {
	srv, getMsgs := mockDiscordServerForOAuth(t, 200)
	defer srv.Close()
	...
	msgs := getMsgs()
	require.GreaterOrEqual(t, len(msgs), 1, "alert should fire for allowed_warning status")
	assert.Contains(t, msgs[0], "allowed_warning", "alert must mention allowed_warning")
}
