...
SELECT COUNT(*) FROM (
  (
    ...
    WHERE sl.starttime >= $1
      ...
      AND ($7::text IS NULL OR sl.upstream_token_key = $7)
      AND ($8::text IS NULL
           OR ($8 = 'success' AND el.id IS NULL)
           OR ($8 = 'failed' AND el.id IS NOT NULL))
  )
  UNION ALL
  (
    ...
    WHERE NOT EXISTS (SELECT 1 FROM "SpendLogs" WHERE request_id = el.request_id)
      ...
      AND ($7::text IS NULL OR el.upstream_token_key = $7)
      AND ($8::text IS NULL OR $8 = 'failed')
...
type CountRequestLogsParams struct {
	...
	FilterUpstreamToken *string            `json:"filter_upstream_token"`
...
SELECT request_id, call_type, api_key, spend, total_tokens, prompt_tokens, completion_tokens, starttime, endtime, completionstartime, model, model_id, model_group, api_base, "user", metadata, cache_hit, cache_key, request_tags, team_id, end_user, requester_ip_address, organization_id, provider, request_duration_ms, upstream_token_key, reasoning_effort
FROM "SpendLogs"
WHERE starttime >= $1 AND starttime < $2
ORDER BY starttime
LIMIT $3
...
(
  ...
  WHERE sl.starttime >= $3
    ...
    AND ($9::text IS NULL OR sl.upstream_token_key = $9)
    AND ($10::text IS NULL
         OR ($10 = 'success' AND el.id IS NULL)
         OR ($10 = 'failed' AND el.id IS NOT NULL))
)
UNION ALL
(
  SELECT
  ...
  WHERE NOT EXISTS (SELECT 1 FROM "SpendLogs" WHERE request_id = el.request_id)
    ...
    AND ($9::text IS NULL OR el.upstream_token_key = $9)
    AND ($10::text IS NULL OR $10 = 'failed')
