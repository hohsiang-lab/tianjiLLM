...
// TestSchemaFilesEmbed verifies that the embed.FS contains exactly 24 .up.sql files.
func TestSchemaFilesEmbed(t *testing.T) {
	...
	assert.Len(t, sqlFiles, 24, "expected exactly 24 .up.sql files in embedded schema FS")
}

// TestSchemaFilesOrder verifies that the iofs source resolves versions 1-24 in order.
func TestSchemaFilesOrder(t *testing.T) {
	...
	assert.Len(t, versions, 24, "expected 24 migration versions")
	...
	assert.Equal(t, uint(24), versions[len(versions)-1], "last migration version must be 24")
}
