...
const listOpenAISubscriptionRefreshCandidates = `-- name: ListOpenAISubscriptionRefreshCandidates :many
SELECT credential_id, credential_name, credential_type, credential_value, credential_info, organization_id, created_at, created_by, updated_at, updated_by FROM "CredentialTable"
...
func (q *Queries) ListOpenAISubscriptionRefreshCandidates(ctx context.Context) ([]CredentialTable, error) {
	rows, err := q.db.Query(ctx, listOpenAISubscriptionRefreshCandidates)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const updateCredentialInfo = `-- name: UpdateCredentialInfo :exec
UPDATE "CredentialTable"
SET credential_info = $2, updated_at = NOW()
WHERE credential_id = $1
`

type UpdateCredentialInfoParams struct {
	CredentialID   string `json:"credential_id"`
	CredentialInfo []byte `json:"credential_info"`
}

func (q *Queries) UpdateCredentialInfo(ctx context.Context, arg UpdateCredentialInfoParams) error {
	_, err := q.db.Exec(ctx, updateCredentialInfo, arg.CredentialID, arg.CredentialInfo)
	return err
}

const updateCredentialValueAndInfo = `-- name: UpdateCredentialValueAndInfo :exec
UPDATE "CredentialTable"
...
func (q *Queries) UpdateCredentialValueAndInfo(ctx context.Context, arg UpdateCredentialValueAndInfoParams) error {
	_, err := q.db.Exec(ctx, updateCredentialValueAndInfo, arg.CredentialID, arg.CredentialValue, arg.CredentialInfo)
	return err
}
