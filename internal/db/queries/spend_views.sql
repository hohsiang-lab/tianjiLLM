...
(
  ...
  WHERE sl.starttime >= sqlc.arg(start_date)
    ...
    AND (sqlc.narg(filter_upstream_token)::text IS NULL OR sl.upstream_token_key = sqlc.narg(filter_upstream_token))
...
(
  SELECT
    ...
    COALESCE(vt2.key_alias, '') AS key_alias,
    el.upstream_token_key,
    el.model,
    el.reasoning_effort,
    0.0::float8         AS spend,
  ...
  WHERE NOT EXISTS (SELECT 1 FROM "SpendLogs" WHERE request_id = el.request_id)
    ...
    AND (sqlc.narg(filter_upstream_token)::text IS NULL OR el.upstream_token_key = sqlc.narg(filter_upstream_token))
