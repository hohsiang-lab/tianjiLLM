...
const (
	...
	// jitterFraction is the max random jitter added as a fraction of delay.
	jitterFraction = 0.25
)
...
// WithModels rebuilds deployments from a new model list while preserving router
// strategy, settings, and registered auto-router callbacks.
func (r *Router) WithModels(models []config.ModelConfig) *Router {
	if r == nil {
		return nil
	}
	r.mu.RLock()
	strategy := r.strategy
	settings := r.settings
	autoRouters := make(map[string]AutoRouterFunc, len(r.autoRouters))
	for prefix, fn := range r.autoRouters {
		autoRouters[prefix] = fn
	}
	r.mu.RUnlock()

	next := New(models, strategy, settings)
	if len(autoRouters) > 0 {
		next.autoRouters = autoRouters
	}
	return next
}
...
func (r *Router) Route(ctx context.Context, modelName string, req *model.ChatCompletionRequest) (*Deployment, provider.Provider, error) {
	...
	if groupPolicy, ok := r.settings.ModelGroupRetryPolicy[modelName]; ok {
		retryPolicy = groupPolicy
		if groupPolicy.NumRetries > 0 {
			numRetries = groupPolicy.NumRetries
		}
	}

	// Apply timeout from retry policy
	if retryPolicy.TimeoutSeconds > 0 {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, time.Duration(retryPolicy.TimeoutSeconds)*time.Second)
		defer cancel()
	}

	tried := make(map[string]bool)
	for attempt := 0; attempt <= numRetries; attempt++ {
		// Exponential backoff between retries (skip for first attempt)
		if attempt > 0 {
			backoff := computeBackoff(attempt-1, retryPolicy)
			select {
...
func (r *Router) ContextWindowFallback(ctx context.Context, modelName string) (*Deployment, provider.Provider, error) {
	...
	for i, fallbackModel := range models {
		if i > 0 {
			select {
		...
		d, p, err := r.Route(ctx, fallbackModel, nil)
		if err == nil {
			return d, p, nil
		}
	}
...
// ListModelGroups returns model group names and their deployments.
...
// computeBackoff calculates the backoff duration for the given attempt.
// Formula: base * 2^attempt, capped at maxBackoff, with random jitter.
func computeBackoff(attempt int, policy RetryPolicy) time.Duration {
	base := defaultBackoffBase
	if policy.RetryAfterSeconds > 0 {
		base = time.Duration(policy.RetryAfterSeconds) * time.Second
	}
	...
	// Add jitter: delay * (1 + rand(0, jitterFraction))
	jitter := delay * rand.Float64() * jitterFraction
