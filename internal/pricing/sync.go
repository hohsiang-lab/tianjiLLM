...
func (e *upstreamModelEntry) UnmarshalJSON(data []byte) error {
	var raw modelInfoJSON
	if err := json.Unmarshal(data, &raw); err != nil {
		return err
	}

	e.InputCostPerToken = raw.InputCostPerToken
	e.OutputCostPerToken = raw.OutputCostPerToken
	e.MaxInputTokens = int(raw.MaxInputTokens)
	e.MaxOutputTokens = int(raw.MaxOutputTokens)
	e.MaxTokens = int(raw.MaxTokens)
	e.Mode = raw.Mode
	e.LiteLLMProvider = raw.Provider
	e.CacheReadInputTokenCost = raw.CacheReadCostPerToken
	e.CacheCreationInputTokenCost = raw.CacheCreationCostPerToken
	e.CacheReadInputTokenCostAbove200k = raw.CacheReadCostPerTokenAbove200k
	e.CacheCreationInputTokenCostAbove200k = raw.CacheCreationCostPerTokenAbove200k
	...
	e.LongContextInputThresholdTokens = threshold.inputTokens
	e.InputCostPerTokenAboveThreshold = threshold.inputRate
	e.OutputCostPerTokenAboveThreshold = threshold.outputRate
	e.CacheReadInputTokenCostAboveThreshold = threshold.cacheReadRate
	e.CacheCreationInputTokenCostAboveThreshold = threshold.cacheCreationRate
	return nil
}

func (e upstreamModelEntry) normalizedLongContextPricing() (threshold int, inputCost, outputCost, cacheReadCost, cacheCreationCost float64) {
	return e.LongContextInputThresholdTokens,
		e.InputCostPerTokenAboveThreshold,
		e.OutputCostPerTokenAboveThreshold,
		e.CacheReadInputTokenCostAboveThreshold,
		e.CacheCreationInputTokenCostAboveThreshold
}
...
func SyncFromUpstream(ctx context.Context, pool *pgxpool.Pool, queries *db.Queries, calc *Calculator, upstreamURL string, openRouterURL string) (int, error) {
	...
	for _, e := range entries {
		longContextThreshold, aboveThresholdInput, aboveThresholdOutput, aboveThresholdCacheRead, aboveThresholdCacheCreation := e.info.normalizedLongContextPricing()
		batch.Queue(upsertModelPricingSQL,
			...
			int32(longContextThreshold),
			aboveThresholdInput,
			aboveThresholdOutput,
			aboveThresholdCacheRead,
			aboveThresholdCacheCreation,
		)
	}
...
func syncEmbeddedFallback(ctx context.Context, pool *pgxpool.Pool, calc *Calculator, existingDB map[string]struct{}) error {
	...
	for _, name := range toInsert {
		...
		batch.Queue(insertEmbeddedFallbackSQL,
			...
			int32(info.LongContextInputThresholdTokens),
			info.InputCostPerTokenAboveThreshold,
			info.OutputCostPerTokenAboveThreshold,
			info.CacheReadCostPerTokenAboveThreshold,
			info.CacheCreationCostPerTokenAboveThreshold,
		)
	}
