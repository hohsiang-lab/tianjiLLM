...
func TestScheduler_AddDoesNotRunAtStartup(t *testing.T) {
	s := New()
	job := &countJob{name: "deferred"}
	s.Add(job, time.Hour)
	...
	assert.Zero(t, job.count.Load(), "Add must wait for its interval instead of running at startup")
}
