...
func (b *RedisBuffer) Push(params db.CreateSpendLogParams) {
	...
	if err := b.rdb.RPush(b.ctx, spendQueueKey, data).Err(); err != nil {
		log.Printf("error: failed to push spend log to Redis queue: %v", err)
	}
}
	...
	for {
		...
		if err != nil {
			if !errors.Is(err, redis.Nil) {
				log.Printf("error: failed to pop spend logs from Redis queue: %v", err)
			}
			return
		}
		if len(results) == 0 {
