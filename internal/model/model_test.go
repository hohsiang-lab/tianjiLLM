...
func TestRequestErrorConstructors(t *testing.T) {
	tests := []struct {
		err    *RequestError
		status int
		param  string
		...
		{UnsupportedParameter("temperature", "unsupported"), 400, "temperature", "unsupported_parameter"},
		{InvalidValue("top_p", "invalid"), 400, "top_p", "invalid_value"},
		{InvalidRequest("max_tokens", "ambiguous"), 400, "max_tokens", "invalid_request"},
		{ModelNotFound("missing"), 404, "", "model_not_found"},
	}

	for _, tt := range tests {
		if tt.err.StatusCode != tt.status || tt.err.Detail.Param != tt.param || tt.err.Detail.Code != tt.code {
			t.Fatalf("got status=%d param=%q code=%q", tt.err.StatusCode, tt.err.Detail.Param, tt.err.Detail.Code)
		}
		if tt.err.Detail.Type != "invalid_request_error" {
			t.Fatalf("type = %q", tt.err.Detail.Type)
		}
	}
}
