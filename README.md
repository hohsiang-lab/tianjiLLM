...
TianjiLLM can route official OpenAI models through admin-connected OpenAI subscription credentials. The OAuth access and refresh tokens stay in TianjiLLM credential storage; downstream clients only use a TianjiLLM API key.
...
Use `/ui/credentials` to connect, test, refresh, disable, or delete subscription credentials. See [OpenAI subscription OAuth admin guide](docs/openai-subscription-oauth.md) for setup and troubleshooting, and [OpenClaw via TianjiLLM gateway](docs/openclaw-tianjillm-gateway.md) for OpenClaw config examples.

### Admin UI social authentication

The admin dashboard supports GitHub and Discord sign-in with OAuth2
authorization code + PKCE. TianjiLLM keeps its own PostgreSQL-backed SCS
session and local user permissions; it does not require deploying a separate
identity service.
...
general_settings:
  ...
  social_auth:
    ...
    allow_verified_email_link: true
...
Social login is invite-only: users must already exist locally, and automatic
first linking requires a unique matching verified email. See the
[social authentication deployment and security guide](docs/social-auth.md)
for provider callbacks, role permissions, rollout, and break-glass recovery.
